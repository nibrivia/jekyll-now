...
state_targets <- data.frame(state = state.abb[match(targets$state, state.name)],
                     ...
                     results = log(results$Clinton_popular_vote/results$Sanders_popular_vote))
state_targets <- melt(state_targets, id.vars = c("state", "results"))

plot <- ggplot(state_targets, aes(x = value, y = results, color=variable) ) +
