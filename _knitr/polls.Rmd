...
d_polldata <- select(end_date, choice, value)

targets <- read.csv("2016-dem-primary-targets-manual.csv")
results <- read.csv("2016-dem-primary.csv")
results <- rename(results, FiveThirtyEight_margins = FiveThirtyEight_target)
...
p_states <- p +
  geom_point(data = targets, mapping=aes(x=date, y = Estimate, fill = choice, color = method), size=2) +
  geom_line(data = targets, mapping=aes(x=date, y = Estimate, fill = choice, color = method), alpha = 0.4) +
  xlab("Date") + ylab("Polls and result (log odds ratio)") +
  facet_wrap(data = targets, ~ method) +
